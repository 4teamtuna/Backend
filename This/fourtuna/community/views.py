...
from .serializers import Community_Serializer
from rest_framework.response import Response
from rest_framework.decorators import api_view
...
def article_list(request):
    ...
    serializer = Community_Serializer(articles, many=True)
...
def board_list(request):
    boards = Article.objects.all().order_by(
        "-id"
    )  # Board모델의 모든 필드를 가져와 id의 역순으로 가져옴(최신글을 맨위로 올림)
